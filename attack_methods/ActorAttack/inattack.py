...
class InAttack:
    def __init__(self, config: InAttackConfig):
        ...
        with open(config.pre_attack_data_path, 'r', encoding='utf-8') as f:
            self.org_data = json.load(f)['data']
        ...
        for actor in data['actors']:
            ...
            for _ in range(3):
                try:
                    ...
                    # Some runs fail to produce any assistant turn with a judge score.
                    final_turn = None
                    ...
                    if final_turn is None:
                        raise RuntimeError("no scored assistant turn generated")

                    data_list.append({
                        "actor": actor,
                        "final_score": final_turn['score'],
                        "final_reason": final_turn['reason'],
                        "dialog_hist": dialog_hist
                    })
                    if final_turn['score'] == 5:
                        is_succeed = True
                    break
                except Exception as e:
