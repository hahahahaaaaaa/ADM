...
def initialize_clients():
    ...
    try:
        gpt_api_key = get_env_variable("GPT_API_KEY") or get_env_variable("OPENAI_API_KEY")
        gpt_base_url = get_env_variable("BASE_URL_GPT") or get_env_variable("OPENAI_BASE_URL")
        if gpt_api_key and gpt_base_url:
            clients['gpt'] = OpenAI(base_url=gpt_base_url, api_key=gpt_api_key)

        claude_api_key = get_env_variable("CLAUDE_API_KEY") or gpt_api_key
        claude_base_url = get_env_variable("BASE_URL_CLAUDE") or gpt_base_url
        if claude_api_key and claude_base_url:
            clients['claude'] = OpenAI(base_url=claude_base_url, api_key=claude_api_key)

        deepseek_api_key = get_env_variable("DEEPSEEK_API_KEY") or gpt_api_key
        deepseek_base_url = get_env_variable("BASE_URL_DEEPSEEK") or gpt_base_url
        if deepseek_api_key and deepseek_base_url:
            clients['deepseek'] = OpenAI(base_url=deepseek_base_url, api_key=deepseek_api_key)

        deepinfra_api_key = get_env_variable("DEEPINFRA_API_KEY") or gpt_api_key
        deepinfra_base_url = get_env_variable("BASE_URL_DEEPINFRA") or gpt_base_url
