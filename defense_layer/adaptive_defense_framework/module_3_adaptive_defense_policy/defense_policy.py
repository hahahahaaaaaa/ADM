...
class AdaptiveDefensePolicy:
    ...
    def __init__(self):
        ...
        self._category_patterns = {
            "cyber": r"\b(hack|hacking|exploit|payload|malware|virus|phishing|credential|network|database|shell|sql|xss|ransomware|keylogger|botnet|rootkit)\b",
            "violence": r"\b(bomb|weapon|firearm|poison|harm|kill|assassinate|explosive|bioweapon|chemical weapon|mass.?casualt\w*|terror\w*|genocide|torture|nerve agent)\b",
            "drugs": r"\b(cocaine|heroin|fentanyl|meth\w*|methamphetamine|opiate|synthesize|synthesis|drug manufactur\w*|illicit drug)\b",
            "fraud": r"\b(identity theft|fraud|scam|steal|money laundering|manipulate stock|counterfeit|blackmail|extort|bribery|insider trading)\b",
            "harassment": r"\b(racism|harass|body shaming|demean|defamatory|violence against|doxx|stalk\w*|discriminat\w*|xenophob\w*)\b",
            "copyright": r"\b(lyrics|lyric|song|generate.{0,20}(passage|excerpt|chapter|scene)|reproduce|verbatim|full text|copyrighted|written by|published by)\b",
        }
        ...
        self._single_turn_harmful_patterns = [
            re.compile(r"\b(hack|malware|virus|phishing|credential|explosive|weapon|poison|ransomware|keylogger|bomb)\b", re.IGNORECASE),
            re.compile(r"\b(fraud|scam|steal|counterfeit|launder|identity theft|blackmail|extort|bribery)\b", re.IGNORECASE),
            re.compile(r"\b(defam\w*|slander|harass\w*|demean|body shaming|xenophob\w*|stalk\w*|discriminat\w*)\b", re.IGNORECASE),
            re.compile(r"\b(cocaine|heroin|fentanyl|meth\w*|methamphetamine|synthesize|synthesis|drug manufactur\w*)\b", re.IGNORECASE),
            re.compile(r"\b(bioweapon|biological weapon|chemical weapon|nerve agent|genocide|torture|suicide|self.harm)\b", re.IGNORECASE),
            re.compile(r"\b(smuggl\w*|traffick\w*|human trafficking|terror\w*|extremis\w*|mass.?shoot\w*|assassin\w*)\b", re.IGNORECASE),
        ]
    ...
    def decide(self, risk_score: float, user_text: str, assistant_text: str, backward_score: float = 0.0, forward_score: float = 0.0, danger_similarity: float = 0.0, cumulative_risk: float = 0.0) -> DefenseDecision:
        ...
        # ── 多轮累积风险动态阈值压缩（MHJ 渐进攻击场景）──────────────────
        # 随对话整体风险积累，逐步降低防御触发阈值，对当前轮风险级别进行升级
        if cumulative_risk >= 0.65:
            # 累积风险极高：全面升级一级（low→medium, medium→high）
            if level == "low" and risk_score >= 0.08:
                level = "medium"
            elif level == "medium":
                level = "high"
        elif cumulative_risk >= 0.45:
            # 累积风险较高：有任意子信号时升级
            if level == "low" and (forward_score >= 0.10 or backward_score >= 0.10 or danger_similarity >= 0.20):
                level = "medium"
            elif level == "medium" and (forward_score >= 0.22 or backward_score >= 0.18 or danger_similarity >= 0.28):
                level = "high"
        elif cumulative_risk >= 0.30:
            # 累积风险中等：明显子信号才升级
            if level == "low" and (forward_score >= 0.15 or backward_score >= 0.14 or danger_similarity >= 0.28):
                level = "medium"
        # ───────────────────────────────────────────────────────────────────
        ...
        if level == "low":
            # 语义危害相似度：嵌入空间接近有害内容时直接拦截
            if danger_similarity >= 0.50:
                return self._sanitize_response(user_text, assistant_text, "medium")
            if danger_similarity >= 0.38:
                return self._guidance(user_text, assistant_text, "medium")
            # backward 高：模型已输出有害内容，强制拦截
            if backward_score >= 0.32:
                return self._sanitize_response(user_text, assistant_text, "medium")
            if backward_score >= 0.14:
                return self._guidance(user_text, assistant_text, "medium")
            # forward 高：用户意图危险，即使模型回复表面无害也拦截
            if forward_score >= 0.42:
                return self._sanitize_response(user_text, assistant_text, "medium")
            if forward_score >= 0.24:
                return self._guidance(user_text, assistant_text, "medium")
            # forward + backward 协同
            if forward_score >= 0.18 and backward_score >= 0.08:
                return self._guidance(user_text, assistant_text, "medium")
            # danger_similarity 中等 + forward 弱信号联合
            if danger_similarity >= 0.28 and forward_score >= 0.10:
                return self._guidance(user_text, assistant_text, "medium")
            # 类别硬规则：已识别为具体危害类别且有任何意图信号
            category = self._detect_category(f"{user_text}\n{assistant_text}")
            if category != "general" and forward_score >= 0.14:
                return self._guidance(user_text, assistant_text, "medium")
            if category != "general" and danger_similarity >= 0.22:
                return self._guidance(user_text, assistant_text, "medium")
            return self._allow(assistant_text, level, strategy="直接放行")

        if level == "medium":
            # 语义危害信号优先于白名单：高 danger_similarity 直接重写
            if danger_similarity >= 0.45:
                return self._sanitize_response(user_text, assistant_text, level)
            if danger_similarity >= 0.32:
    ...
    def _guidance(self, user_text: str, assistant_text: str, level: str) -> DefenseDecision:
        category = self._detect_category(f"{user_text}\n{assistant_text}")
        # 版权类：完全不保留原始内容；其他已知有害类别强过滤；general 类弱过滤
        if category == "copyright":
            safe_text, removed_count = "", 1
        else:
            aggressive_filter = category != "general"
            safe_text, removed_count = self._extract_safe_information(assistant_text, aggressive=aggressive_filter)
        notice = "提示：请仅在合规场景下参考下述信息。"

        preserved = safe_text if safe_text else ""
        parts: List[str] = []
        if preserved:
